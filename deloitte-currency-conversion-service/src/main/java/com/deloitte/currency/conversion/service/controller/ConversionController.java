...
public class ConversionController {
	...
	public ResponseEntity<ConversionBean> getConvertedValue(
			@PathVariable("from")String from,
			@PathVariable("to")String to,
			@PathVariable("quantity")float quantity){
		
		ResponseEntity<ConversionBean> cbResponse=rt.getForEntity("http://localhost:8500/exchange/from/"+from+"/to/"+to,ConversionBean.class);
